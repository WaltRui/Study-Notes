...
而类中其他方法，则是依托这几个方法的结果进行查询，对使用什么框架没影响。所以在子类中，只需重新实现这五个方法，而其他方法无需变动。
这种情况下需要使用Abstract 修饰符。
...
### 2.  virtual和new
#### 介绍
virtual关键字表示方法可被子类覆盖。但是即使不使用virtual，子类中编写一个同名方法（返回值和参数一致）也是可以覆盖基类方法的。
...
### 4. Assembly
程序集，定义在System.Reflection命名空间下。用来加载和执行程序集，可以认为每个项目就是一个程序集。
#### Abp中使用
...
### 8. IQueryable, IEnumerable, IList
三个集合接口，常用在EF查询中。
...
需求：求一个Product集合（IList或IQueryable类型）的价格总和。IList中不可能有这个方法，因为无法确定集合元素类型，以及对哪个属性求和。
但是可以使用扩展方法。
